...
resource "aws_iam_role" "cross_account_role" {
  ...
  tags = {
    Name = "${var.workspace_name}-crossaccount-role"
  }
}

resource "aws_iam_role_policy" "cross_account" {
  name = "${var.workspace_name}-crossaccount-policy"
  ...
  policy = jsonencode({
    ...
    "Statement" : [
      ...
      },
      {
        "Effect" : "Allow",
        "Action" : "iam:PassRole",
        "Resource" : "arn:aws:iam::${var.aws_account_id}:role/${var.workspace_name}-s3-instance-profile"
      }
    ]
    }
  )
